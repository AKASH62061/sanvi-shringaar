...
router.get("/", async (req, res) => {
  try {
    ...
    if (term) {
      // Escape regex special characters so symbols in a search term don't
      // break the query, then match anywhere in the name/brand/description
      // (case-insensitive, partial matches included) instead of relying on
      // MongoDB's $text operator, which only matches whole, stemmed words
      // and misses simple partial searches like "lip" for "Lipstick".
      const escaped = term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const regex = new RegExp(escaped, "i");
      const matchingCategories = await Category.find({ name: regex }, "_id");
      filter.$or = [
        { name: regex },
        { brand: regex },
        { description: regex },
