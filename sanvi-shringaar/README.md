...
- **Family login required** — a single shared username/password lets everyone
  sign in to browse sections (Lipstick, Bindi, Bangles, etc.) and see each
  item's photo and price range (min–max). The link isn't useful to anyone
  without the login.
- **Add or update** — once signed in, whoever you choose can add categories,
  add items, upload photos, and edit prices from the Admin panel.
...
- **Everyone you share the link with** needs to sign in with the shared
  `ADMIN_USERNAME`/`ADMIN_PASSWORD` first — click **Family Login** (top
  right). Once signed in, use the search bar in the navbar (or on the home
  page) to find an item's price instantly, or browse by section.
- **Adding or editing prices**: once signed in, go to **Admin**:
...
- Everyone who has the `ADMIN_USERNAME`/`ADMIN_PASSWORD` can both browse and
  edit — there isn't a separate "view only" login in this version.
